    ...
    for period in periods:
        ...
        for w_snap in want_snaps:
            if w_snap in managed_snaps.keys():
                ...
                if period == 'day' and not datetime.strptime(w_snap, '%d-%m-%Y').strftime('%a') == config['week_start'] and not datetime.strptime(w_snap, '%d-%m-%Y').strftime('%d').lstrip('0') == str(config['month_start']):
                    managed_snaps.pop(w_snap)
                if period == 'week' and not datetime.strptime(w_snap, '%d-%m-%Y').strftime('%d').lstrip('0') == str(config['month_start']):
                    managed_snaps.pop(w_snap)
                if period == 'month':
                    managed_snaps.pop(w_snap)
            else:
...
# Check which snapshot period to run for, which snapshot should be deleted and set the date_suffix to use in
    ...
    for vol in vols:
        try:
            ...
            try:
                ...
                continue
            deleted = False
            # Do not try to delete snapshots which fulfills another period. While without this check the there is
            # no risk of deleting a snapshot that is still required, a warning message would be produced where
            # no matching snapshot is found.
            if (period == 'day'\
                and (\
                    ...
                    if snap.description.startswith(period):
                        if delete_target in snap.description:
                            try:
                                ...
                                print(f'Error: Could not delete \'{period}\' snapshot {snap}: {e}')
                                logger.error(f'Could not delete \'{period}\' snapshot {snap}: {e}')
                                count_errors += 1
                                break
                if not deleted:
                    # Log a warning if the expected snapshot does not exist
                    print(f'Warning: Could not find \'{period}\' snapshot {delete_target} for {vol_name} or the snapshot could not be deleted')
